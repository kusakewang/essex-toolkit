...
This script shows how to use the fnllm batching functionality w        # Note: These would be used if we actually ran the batcher with a valid API key
...
async def main():
    ...
    # Large text handling example
    print(f"\n📄 Large Text Handling:")
    print("-" * 25)

    large_text = "This is a very long text. " * 1000  # Create a large text
    large_text_tokens = text_service.count_tokens(large_text)

    print(f"   Large text tokens: {large_text_tokens}")
    print(f"   Max batch tokens: {max_batch_tokens}")

    if large_text_tokens > max_batch_tokens:
        print(f"   ✂️  Large text will be automatically split and averaged")

        # Process the large text
        large_future = sdk_batcher(large_text)
        large_result = await large_future
        print(f"   ✅ Large text processed successfully")
        print(f"   Result dimensions: {len(large_result)}")
...
async def demonstrate_error_handling():
    """Demonstrate error handling in batching scenarios."""
    print("\n🛡️  Error Handling Demo:")
    print("-" * 25)

    # This would normally fail with invalid API key
    try:
        config = PublicOpenAIConfig(
            api_key="invalid-key",
            base_url="https://api.openai.com",
        ...
        # Note: These would be used if we actually ran the batcher
        # llm = create_openai_embeddings_llm(config)
        # text_service = OpenAITextService(tiktoken.get_encoding("cl100k_base"))

        # Note: Not actually running this due to invalid API key
        # batcher = OpenAIEmbeddingBatcher(
        #     llm=llm,
        #     text_service=text_service,
        #     max_batch_size=2,
        ...
        # This would fail due to invalid API key
        # await batcher.batch_invoke(["Test text"])

        print("   ℹ️  Error handling is built into the LLM implementations")
        print("   ℹ️  Both SDK and REST versions handle API errors gracefully")
...
if __name__ == "__main__":
    ...
    if os.getenv("OPENAI_API_KEY") is None:
        ...
        print("\n   This example will demonstrate the API without actually calling OpenAI")
